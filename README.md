...
2. **Create your local config and add your Anthropic API key.**
   `config.json` holds your API keys, so it's git-ignored and never
   committed. Copy the tracked template and fill in a real key:
   ...
   Never commit `config.json` once it has real keys in it.
...
config.example.json    # tracked template -- copy to config.json and fill in
config.json            # your local API keys, model, port, boss name (git-ignored)
viewer/
